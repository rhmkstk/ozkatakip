...
const buildEditPayload = () => {
  const manufactureYear = editForm.manufacture_year
    ? new Date(`${editForm.manufacture_year.getFullYear()}-01-01`)
    : null;
  return {
    id: editForm.id,
    ...
    next_refill_date: editForm.next_refill_date,
    hydrostatic_test_date: editForm.hydrostatic_test_date,
    next_hydrostatic_test_date: editForm.next_hydrostatic_test_date,
...
const getEditSnapshot = () => {
  ...
  return JSON.stringify({
    ...
    manufacture_year: payload.manufacture_year
      ? new Date(payload.manufacture_year).toISOString()
      : null,
    refill_date: payload.refill_date
      ? new Date(payload.refill_date).toISOString()
      : null,
    next_refill_date: payload.next_refill_date
      ? new Date(payload.next_refill_date).toISOString()
      : null,
    hydrostatic_test_date: payload.hydrostatic_test_date
      ? new Date(payload.hydrostatic_test_date).toISOString()
      : null,
    next_hydrostatic_test_date: payload.next_hydrostatic_test_date
      ? new Date(payload.next_hydrostatic_test_date).toISOString()
      : null,
  });
};
    ...
    <div class="space-y-6">
      ...
      <div class="form-row">
        <div class="form-item">
          ...
          <InputText id="edit-brand" v-model="editForm.brand" />
        </div>
        <div class="form-item">
