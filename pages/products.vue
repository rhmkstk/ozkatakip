...
<template>
  <div>
	...
	<EmptyState v-if="!tabledata.length">
        <template #title>YSC bulunamadı</template>
        <template #subtitle>YSC'leri listelemek için önce eklemelisiniz.</template>
		<template #footer>
			<Button variant="text" class="w-full mt-2" label="YSC Ekle" @click="$router.push('/insert-data')"></Button>
		</template>
      </EmptyState>
